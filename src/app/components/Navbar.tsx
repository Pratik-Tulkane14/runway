...
const Navbar = () => {
    const style = "absolute top-0 z-10 h-16 w-full px-5 py-3 flex justify-between items-center transition-all duration-200 ease-in-out";
        ...
        <>
            <div className={`w-full ${isScrolled ? "bg-[rgba(0,0,0,0.9)]" : "bg-transparent"} ${style}`}>
                ...
                <button className='md:hidden' onClick={handleClick}>
                    {!isMenuOpen && <IoMenu size={30} className='text-white' />
            ...
            <div className={`absolute  ${isMenuOpen ? "-left-0" : "-left-full"} bg-white transition-all duration-300 ease-in-out w-full h-screen z-20 px-5 py-3`}>
                <div className="flex justify-between items-center">
                    ...
                    <IoClose size={30} className='text-black' onClick={handleClick} />
                </div>
